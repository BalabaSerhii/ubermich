export default function PrimaryAdvantages() {
  const advantages = [
    {
      title: "Marketing",
      description:
    ...
    <section className="dark:bg-gray-900">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {advantages.map((advantage, index) => (
            <div key={index} className="group">
              <div className="flex justify-center items-center mb-4 w-10 h-10 rounded-full bg-(--primary) lg:h-12 lg:w-12 dark:bg-(--primary) group-hover:bg-primary-200 dark:group-hover:bg-primary-800 transition-colors duration-200">
                <IconWrapper>{advantage.icon}</IconWrapper>
              </div>
              <h3 className="mb-2 text-xl font-bold dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors duration-200">
                {advantage.title}
              </h3>
      ...
      <div className="mt-8">
        <div className="bg-(--primary) text-white p-4 rounded-lg text-center">
          Тестовый блок - если этот текст белый на синем фоне, то цвета работают
        </div>
      </div>
    </section>
  );
